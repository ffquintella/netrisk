...
- **Track 6 — `DAL.IntegrationTests` harness**: new test project using Testcontainers (`Testcontainers.MySql`) that boots a throwaway MySQL 8.0 container to verify the shipped `schema_upgrade_log` DDL, the EF entity round-trip, and the full apply orchestration (apply + validate + audit-log, plus the validation-failure path) end-to-end against real MySQL. Tagged `Category=Integration` (requires Docker; excludable from the fast unit run). Unit coverage for the tool is 29 tests in `ServerServices.Tests`. See [roadmap/track-6/MILESTONE_6.1_TOOLING_PREPARATION.md](roadmap/track-6/MILESTONE_6.1_TOOLING_PREPARATION.md).
