...
- **Track 6 — `DAL.IntegrationTests` harness**: new test project using Testcontainers (`Testcontainers.MySql`) that boots a throwaway MySQL 8.0 container to verify the shipped `schema_upgrade_log` DDL, the EF entity round-trip, the full apply orchestration (apply + validate + audit-log, plus the validation-failure path), and the baseline census end-to-end against real MySQL. Tagged `Category=Integration` (requires Docker; exclude from the fast unit run with `--filter "Category!=Integration"`). Unit coverage for the tool is 31 tests in `ServerServices.Tests`.
