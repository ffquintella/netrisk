...
- **Track 6 — `netrisk-console database upgrade-schema` command**: new ConsoleClient operation with `--phase`, `--env`, `--check`, `--dry-run`, `--yes`, and `--output`. `--check` runs read-only pre-flight (connectivity, current-vs-expected `db_version`, phase SQL-file presence, and the destructive `6b` observation-window gate against `schema_upgrade_log`); `--dry-run` prints/writes the exact numbered SQL a phase would apply. Both mutate nothing. The destructive apply path is intentionally gated pending the Testcontainers integration harness. Backed by `ISchemaUpgradeService`/`SchemaUpgradeService` and the pure `SchemaUpgradePlanner`/`SchemaUpgradeManifestLoader`, covered by 28 unit tests (manifest validation incl. the shipped file, version/file planning, and the check/dry-run/gate logic). See [roadmap/track-6/MILESTONE_6.1_TOOLING_PREPARATION.md](roadmap/track-6/MILESTONE_6.1_TOOLING_PREPARATION.md).
