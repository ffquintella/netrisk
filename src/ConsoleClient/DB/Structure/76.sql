...
CREATE TABLE IF NOT EXISTS `incident_response_plan_task_dependencies` (
...
CREATE UNIQUE INDEX IF NOT EXISTS `uq_irptd_task_depends_on`
    ON `incident_response_plan_task_dependencies` (`task_id`, `depends_on_task_id`);

CREATE INDEX IF NOT EXISTS `idx_irptd_depends_on_task_id`
...
ALTER TABLE `incident_response_plan_task_dependencies`
    ADD CONSTRAINT `fk_irptd_task_id` FOREIGN KEY IF NOT EXISTS (`task_id`)
        REFERENCES `incident_response_plan_tasks` (`Id`) ON DELETE CASCADE;

ALTER TABLE `incident_response_plan_task_dependencies`
    ADD CONSTRAINT `fk_irptd_depends_on_task_id` FOREIGN KEY IF NOT EXISTS (`depends_on_task_id`)
        REFERENCES `incident_response_plan_tasks` (`Id`) ON DELETE CASCADE;
...
ALTER TABLE `incident_response_plan_tasks` ADD COLUMN IF NOT EXISTS `override_reason` text
    CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL;

ALTER TABLE `incident_response_plan_tasks` ADD COLUMN IF NOT EXISTS `overridden_by_id` int(11) NULL;

ALTER TABLE `incident_response_plan_tasks` ADD COLUMN IF NOT EXISTS `overridden_at` datetime NULL;

CREATE INDEX IF NOT EXISTS `idx_irpt_overridden_by_id`
    ON `incident_response_plan_tasks` (`overridden_by_id`);

ALTER TABLE `incident_response_plan_tasks`
    ADD CONSTRAINT `fk_irpt_overridden_by_id` FOREIGN KEY IF NOT EXISTS (`overridden_by_id`)
