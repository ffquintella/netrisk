...
-- Guarded so re-running this version converges: the column is already named `Solution`.
SET @nr_ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS
                   WHERE TABLE_SCHEMA = DATABASE() AND BINARY TABLE_NAME = 'Incidents' AND BINARY COLUMN_NAME = 'Solution') > 0,
                 'DO 0', 'ALTER TABLE `Incidents` RENAME COLUMN `Resolution` TO `Solution`');
