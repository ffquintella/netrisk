...
- **Replace the EF Core MySQL provider:** Swap `Pomelo.EntityFrameworkCore.MySql` (`src/DAL/DAL.csproj`) for `Microting.EntityFrameworkCore.MySql`, a fork maintained to track each EF Core release more closely than Pomelo's own cadence — relevant since this repo already runs EF Core 10 (see [global.json](global.json)). Needs a full run of `DAL.IntegrationTests` (Testcontainers MariaDB) plus the numbered-SQL upgrade suite (`SchemaUpgradeIdempotenceTest`, `SchemaUpgradeTableReferencesTest`, `DAL.IntegrationTests/SchemaUpgradeRetryTests`) against the new provider before it can replace Pomelo, since those tests assert on MariaDB-specific SQL generation.
